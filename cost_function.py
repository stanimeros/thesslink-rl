...
def cost_components(
    ...
    """Raw (distance, privacy, energy) for one POI. cost = w_d*d + w_p*p + w_e*e"""
...
def cost_function(
    ...
    return w_distance * d + w_privacy * p + w_energy * e
...
def suggest_poi(
    ...
    w_d, w_p, w_e = w
    costs = [
        sum(wi * c for wi, c in zip((w_d, w_p, w_e), cost_components(p, agent_pos, human_pos, grid_size)))
        for p in pois
    ]
    return int(np.argmin(costs))


def save_weights(weights: Tuple[float, float, float], path: Path | str = WEIGHTS_FILE) -> None:
    """Save weights to JSON file."""
    data = {"distance": weights[0], "privacy": weights[1], "energy": weights[2]}
    with open(path, "w") as f:
        json.dump(data, f, indent=2)


def load_weights(path: Path | str = WEIGHTS_FILE) -> Tuple[float, float, float]:
    """Load weights from JSON file. Returns defaults if file missing."""
    if not os.path.exists(path):
        return (1.0 / 3, 1.0 / 3, 1.0 / 3)
    with open(path) as f:
        data = json.load(f)
    return (data["distance"], data["privacy"], data["energy"])
...
def train_and_save(
    ...
    for i in range(n_iterations):
        ...
        d, p, e = cost_components(pois[suggested], agent_pos, human_pos, grid_size)
        grad = np.array([d, p, e], dtype=np.float64)
        ...
        if (i + 1) % log_every == 0:
            print(f"iter {i+1:5d} | cost={cost:.4f} | w=[d:{weights[0]:.3f} p:{weights[1]:.3f} e:{weights[2]:.3f}]")

    w_tuple = (float(weights[0]), float(weights[1]), float(weights[2]))
...
if __name__ == "__main__":
    ...
    if args.no_train:
        w = load_weights()
        print(f"Loaded weights: distance={w[0]:.3f}, privacy={w[1]:.3f}, energy={w[2]:.3f}")
    else:

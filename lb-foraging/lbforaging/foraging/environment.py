...
class ForagingEnv(gym.Env):
    ...
    def __init__(
        ...
        penalty=0.0,
        render_mode=None,
        allow_agent_on_food=False,
        allow_agent_on_agent=False,
    ):
        self.logger = logging.getLogger(__name__)
        self.render_mode = render_mode
        self.allow_agent_on_food = allow_agent_on_food
        self.allow_agent_on_agent = allow_agent_on_agent
    ...
    def _is_valid_action(self, player, action):
        def _cell_ok(row, col):
            if row < 0 or row >= self.rows or col < 0 or col >= self.cols:
                return False
            return (
                self.field[row, col] == 0
                or (self.allow_agent_on_food and self.field[row, col] > 0)
            )
        ...
        if action == Action.NONE:
            ...
            return (
                player.position[0] > 0
                and _cell_ok(player.position[0] - 1, player.position[1])
            )
            ...
            return (
                player.position[1] > 0
                and _cell_ok(player.position[0], player.position[1] - 1)
            )
    ...
    def step(self, actions):
        ...
        for k, v in collisions.items():
            if len(v) > 1 and not self.allow_agent_on_agent:
                continue  # block when multiple players target same cell
            for player in v:
                player.position = k

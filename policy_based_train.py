...
Train POI suggestion policy with Reinforcement Learning (PPO).
  ...
  python policy_based_train.py                    # Train 50k timesteps, save model
  python policy_based_train.py --steps 100000      # More training
  python policy_based_train.py --no-train          # Just evaluate loaded model
"""
...
class PlottingCallback(BaseCallback):
    ...
    def _on_training_end(self) -> None:
        ...
        axes[1].plot(steps, self.cumulative_reward_history, color="tab:cyan", linewidth=0.8, alpha=0.9, marker="o", markersize=3)
        axes[1].set_ylabel("Cumulative Reward")
        axes[1].set_title("Cumulative Reward")
        axes[1].grid(True, alpha=0.3)

        if self.agreement_history:
            axes[2].plot(steps, self.agreement_history, color="tab:blue", linewidth=0.8, alpha=0.9, marker="o", markersize=3)
            axes[2].set_ylabel("Agreement with nearest-human baseline")
            axes[2].set_ylim(0, 1.05)
        else:
            axes[2].plot(steps, self.reward_history, color="tab:blue", linewidth=0.8, alpha=0.9)
...
def suggest_poi_rl(
    ...
    if model_path is not None:
        ...
        for candidate in (_best_model_path, MODEL_PATH):
            ...
            print("Model not found. Run policy_based_train.py first to train the RL policy.")
            sys.exit(1)
...
def evaluate_vs_baseline(
    ...
    """Compare RL policy vs nearest-human baseline. Returns agreement rate."""
...
def main():
    ...
    parser.add_argument("--no-plot", action="store_true", help="Skip generating training_plot_ppo.png")
    ...
    if args.no_train:
        ...
        print("Evaluating RL policy vs nearest-human baseline...")
